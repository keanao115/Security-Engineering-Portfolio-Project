...
|---|---|---|---|
| **Local Host Discovery** | `REAL` | `server/src/services/localHostDiscovery.ts` | Queries OS kernel via `os.networkInterfaces()`, retrieves real local IP/subnet, identifies loopback, and computes CIDR boundaries. |
| **TCP Connect Port Scanner** | `REAL` | `server/src/services/tcpPortScanner.ts` | Initiates genuine TCP three-way handshake (`net.Socket.connect`), enforces explicit timeout controls, validates target hosts against SSRF boundaries. |
| **ARP Discovery** | `REAL / OS Dependent` | `server/src/services/arpDiscovery.ts` | Parses OS ARP table via native `arp -a` CLI execution; explicitly flags OS platform limitations and returns genuine MAC-to-IP pairings. |
| **Netstat / Active Sockets** | `REAL / OS Dependent` | `server/src/services/netstatCollector.ts` | Executes OS native socket inspection (`netstat -ano` / `ss -tulpn`) and parses active TCP/UDP listening states without fabrication. |
...
| **Nmap XML Parser** | `REAL` | `server/src/services/nmapXmlParser.ts` | Ingests genuine Nmap `-oX` standard XML reports via `fast-xml-parser`, extracting open ports, service banners, CPEs, and host scripts. |
...
| **Syslog Collector (RFC 5424/3164)** | `REAL` | `server/src/collectors/syslog/syslogCollector.ts` | Binds real UDP/TCP sockets (default port 5140/514), parses RFC 5424 PRI/Facility/Severity headers and ISO timestamps. |
| **WEF / Windows Event Collector** | `PARTIAL` | `server/src/collectors/wef/wefCollector.ts` | Ingests Windows Event XML / JSON payloads, standardizing EventID 4624 (Logon), 4625 (Failed Logon), 4688 (Process Creation) into unified schema. |
| **NetFlow v5/v9 Decoder** | `REAL` | `server/src/collectors/netflow/netflowCollector.ts` | Decodes binary NetFlow datagrams, unpacking 5-tuple metrics, packet counts, byte counts, and autonomous system numbers. |
...
| **Threat Intel Feeds & CISA KEV** | `REAL` | `server/src/services/threatFeedService.ts` | Synchronizes with CISA Known Exploited Vulnerabilities catalog, AlienVault OTX, and Tor exit node lists with provenance headers. |
| **Sigma Detection Rule Engine** | `REAL` | `server/src/correlation/correlationEngine.ts` | Evaluates detection rules against unified telemetry event streams using pattern matching, condition trees, and severity classification. |
...
| **RAG Knowledge Base** | `PARTIAL` | `server/src/services/ragService.ts` | Local threat intelligence corpus embedding and semantic retrieval grounded in vetted MITRE ATT&CK techniques and incident playbooks. |
| **Attack Simulation Suite** | `SIMULATED` | `server/src/demo/syntheticFlowGenerator.ts` | Explicitly marked as synthetic attack scenarios (Port Scan, Brute Force, Data Exfiltration) for red/blue SOC training and verification. |
